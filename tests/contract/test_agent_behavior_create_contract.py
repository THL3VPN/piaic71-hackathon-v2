...
# [Task]: T004 [From]: specs/014-agent-behavior-validation/spec.md User Story 1
...
def test_behavior_create_instructions_include_add_tool(auth_headers_factory, monkeypatch) -> None:
    responses = [
        build_response(
            build_message(
                content="",
    ...
    async def _fake_request_completion(*, client, model_name, messages, tool_specs):
        ...
        assert "add_task" in system_content
        assert "create" in system_content or "add" in system_content
    ...
    headers = auth_headers_factory("behavior-create-user")
