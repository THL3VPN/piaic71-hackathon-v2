...
# [Task]: T005 [From]: specs/013-agent-tool-calls/spec.md User Story 1
    ...
    responses = [
        _FakeResponse(
            _FakeMessage(
                content="",
                tool_calls=[
                    _FakeToolCall("call-1", "add_task", '{"title":"prepare report"}')
                ],
            )
        ),
...
def test_agent_tool_calls_contract(auth_headers_factory, monkeypatch) -> None:
    _setup_fake_agent(monkeypatch)
    headers = auth_headers_factory("agent-tool-contract-user")
    with TestClient(app) as client:
        resp = client.post(
            "/api/chat",
            headers=headers,
            json={"message": "Create a task called prepare report"},
        )
...
# [Task]: T012 [From]: specs/013-agent-tool-calls/spec.md User Story 2
def test_agent_tool_calls_payload_shape(auth_headers_factory, monkeypatch) -> None:
    _setup_fake_agent(monkeypatch)
    headers = auth_headers_factory("agent-tool-contract-user")
    with TestClient(app) as client:
        resp = client.post(
