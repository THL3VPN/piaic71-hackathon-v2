...
# [Task]: T006 [From]: specs/015-mcp-server-extraction/spec.md User Story 1
...
@pytest.mark.anyio
async def test_mcp_tool_operations(tmp_path) -> None:
    ...
    _, created = await mcp.call_tool(
        "add_task",
        {"user_id": "integration-user", "title": "File taxes"},
    )
    task_id = created["task_id"]

    _, listed_raw = await mcp.call_tool("list_tasks", {"user_id": "integration-user", "status": "pending"})
    listed = _unwrap_list(listed_raw)
    assert any(task["id"] == task_id for task in listed)

    _, updated = await mcp.call_tool(
        "update_task",
        {"user_id": "integration-user", "task_id": task_id, "title": "File taxes ASAP"},
    )
    assert updated["title"] == "File taxes ASAP"

    _, completed = await mcp.call_tool(
        "complete_task",
        {"user_id": "integration-user", "task_id": task_id},
    )
