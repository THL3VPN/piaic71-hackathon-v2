...
# [Task]: T006 [From]: specs/013-agent-tool-calls/spec.md User Story 1
...
def test_agent_tool_execution_creates_task(auth_headers_factory, monkeypatch) -> None:
    responses = [
        _FakeResponse(
            _FakeMessage(
                content="",
                tool_calls=[
                    _FakeToolCall("call-1", "add_task", '{"title":"file expenses"}')
                ],
            )
        ),
    ...
    headers = auth_headers_factory("agent-tool-api-user")
    with TestClient(app) as client:
        resp = client.post(
            "/api/chat",
            headers=headers,
            json={"message": "Create a task called file expenses"},
        )
        ...
        titles = [task["title"].lower() for task in tasks_resp.json()]
        assert "file expenses" in titles
